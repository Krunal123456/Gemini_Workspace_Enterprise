...
- [ ] One canonical `:root` — no token defined twice with conflicting values
- [ ] Component colors reference semantic tokens, not raw literals
- [ ] Light and dark define the same roles
- [ ] Hover / focus / disabled reuse the scale, no new one-off colors
- [ ] Color notation is consistent across the system (OKLCH is the default; a consistent hex or rgb system also passes)
