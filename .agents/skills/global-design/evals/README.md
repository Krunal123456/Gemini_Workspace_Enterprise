...
| File | Purpose |
|---|---|
| `trigger-evals.json` | 24 prompts with `should_trigger: true/false` — tests skill routing |
...
| `fixtures/` | Two HTML pages for the rendered-verification runner: one with planted Gate 3/5/6/7 defects, one clean |

## Self-test for the rendered verifier

`scripts/verify-rendered.mjs` (paths here resolve from the skill root) is the scripted half of Decision Pipeline step 12. The fixtures prove it still catches what it claims to catch — run both after touching the script:

```bash
node scripts/verify-rendered.mjs "file:///ABS/SKILL/ROOT/evals/fixtures/rendered-defects.html"   # expect exit 1
node scripts/verify-rendered.mjs "file:///ABS/SKILL/ROOT/evals/fixtures/rendered-clean.html"     # expect exit 0
```

`rendered-defects.html` plants one of each: horizontal overflow at 390px, a heading hidden by `-webkit-text-fill-color: transparent`, a 20×20 button, an image without `width`/`height`, `min-height: 100vh`, and hex/rgb colors where OKLCH belongs. Use an absolute `file:///C:/...` URL — a Git Bash `$(pwd)` path produces `file:///c/...`, which Chromium cannot open, and the run fails on load instead of on a check.
