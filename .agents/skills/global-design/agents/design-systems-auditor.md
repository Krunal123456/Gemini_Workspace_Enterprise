...
grep -rn "padding:[[:space:]]*[0-9]px\|margin:[[:space:]]*[0-9]px\|gap:[[:space:]]*[0-9]px" src/ --include="*.css"

# Find hardcoded px font sizes
grep -rn "font-size:[[:space:]]*[0-9]*px" src/ --include="*.css"

# Find hardcoded border-radius not from token
grep -rn "border-radius:[[:space:]]*[0-9]" src/ --include="*.css"
...
*Agent version: global-design-skill v1.0*
*Related: `tokens/tokens.css`, `tokens/tokens-dark.css`, `rules/04-color.md`, `rules/02-layout-and-grid.md`, [`examples/02-color-token-migration.md`](https://github.com/staurus86/global-design-skill/blob/master/examples/02-color-token-migration.md)*
